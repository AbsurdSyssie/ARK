...
nrow(node2a)/ (nrow(node2a)+nrow(node2b))*100 #% of single rx which were stopped within 72hrs
  ...
  #was the initial rx a double rx
  node2c <- node1b %>% #yes
...
nrow(node3a)/ (nrow(node3a)+nrow(node3b))*100 #% of rx courses >=2 that had the first prescription as IV
      ...
      node3d <- node2c %>% 
        filter(is.na(Name_3)) %>% remove_all_na() #no
