...
#remove any stat, bleed, prophylactic abx, leg cramps, hepatic encephalopathy
...
abx_per_indication <- abx_per_indication %>% 
  mutate(Decision_1 = case_when(
    Route_1 =="Intravenous" & Route_2 =="Oral" & Prescription.start_2 > Prescription.end_1 ~ 'IV to Oral Switch',
    Route_1 =="Oral" & Route_2 =="Intravenous" & Prescription.start_2 > Prescription.end_1  ~ 'Oral to IV Switch',
...
#make logic for a decision "X agent added"
abx_per_indication <- abx_per_indication %>%  mutate(Decision_1 = case_when( 
Route_1 == "Intravenous" & Route_2 == "Intravenous" & interval(Prescription.start_1, Prescription.start_2) / hours(1) < 12 & Prescription_Length_1 > 12 ~ 'Dual IV therapy started',
 Route_1 == "Intravenous" &  Route_2 == "Oral" & interval(Prescription.start_1, Prescription.start_2) / hours(1) < 12 & Prescription_Length_1 > 12 ~ 'IV and Oral started',
TRUE ~ "uh-oh"
  ))
...
course_lengths <- course_lengths %>%  mutate(course_length =  case_when(
...
```{r}
#get all courses where the first decision was not "finalised" or "No Change" i.e the drug was switched
rx_switched <- ark_results %>%  filter(!grepl("Finalised", Decision_1) | (!grepl("No Change", Decision_1)))
...
percent_in_week_switched %>% 
    ...
    ylim(0,100)+
    labs(x = "Month", y = "Percentage of rx <= 72 hours", title = "Antibiotic Courses Switched Within 72 Hours, Lilac Ward Dec '21 - Mar '22", subtitle = "Including both oral and IV regiments") +
    geom_bar(stat = "identity", colour = "black", fill = "pink")
```
...
```{r}
#get all courses where the first decision was not "finalised" or "No Change" i.e the drug was switched
rx_switched <- ark_results %>%  filter(grepl("IV to Oral Switch", Decision_1))
#floor by week to group them together
rx_switched$Prescription.start_1 <- floor_date(rx_switched$Prescription.start_1, "week")
#then take those and count how many there were each week
rx_switched_week <- rx_switched %>%  group_by(Prescription.start_1) %>%  count(Prescription.start_1)
...
rx_switched_short_week <- rx_switched_short %>%  group_by(Prescription.start_1) %>%  count(Prescription.start_1)
#then compare them 
percent_in_week_switched <- data.frame(rx_switched_week$Prescription.start_1) %>%  mutate(percent_short = rx_switched_short_week$n/rx_in_week[1:13,2] *100)
