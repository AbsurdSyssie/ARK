...
```{r}
rx_under_week_stopped <- select(single_rx, c(Prescription.start_1,Prescription_Length_1))
...
```{r}
rx_in_week <- select(Clean_Data, c(Prescription.start,Prescription_Length))
...
```{r}
percent_in_week <- data.frame(rx_in_week$Prescription.start) %>%  mutate(percent_short = rx_under_week_stopped$n/rx_in_week$n *100)
```
