...
nrow(node3a)/ (nrow(node3a)+nrow(node3b))*100 #% of rx courses >=2 that had the first prescription as single IV
      ...
      node3c <- node2c %>% 
        filter(!is.na(Name_3)) %>% remove_all_na() #double rx which had more than 3 rx in the course
      
      node3d <- node2c %>% 
        filter(is.na(Name_3)) %>% remove_all_na() #double rx which only had the initial double 
      
      #if there was one rx and it was rx for less than 72 hours, was it IV?
      node3e <- node2a %>% 
        filter(Route_1 == "Intravenous") %>% remove_all_na() #single rx, short, IV - outcome stopped within 72 hrs
      node3f <- node2a %>% 
        filter(!Route_1 == "Intravenous") %>% remove_all_na()    #single rx, short, oral - outcome stopped within 72hrs
      ...
      {  node4a <- node3c %>% 
          filter(Route_1 == Route_2) %>% remove_all_na() #double rx, course >=3, same initial route 
        node4b <- node3c %>% 
          filter(!Route_1 == Route_2) %>% remove_all_na() #double rx, course >=3, mixed route
        ...
        node4d <- node3d %>% 
         ...
         node4e <- node3a %>% 
          filter(Prescription_Length_1 <=72) %>% remove_all_na() #single initial rx, course >1, IV, reviewed <72
         node4f <- node3a %>% 
          filter(Prescription_Length_1 >72) %>% remove_all_na() #single initial rx, course >1, IV, reviewed >72
         node4g <- node3b %>% 
          filter(Prescription_Length_1 <=72) %>% remove_all_na() #single initial rx, course >1, oral, reviewed <72
         node4h <- node3b %>% 
          filter(Prescription_Length_1 >72) %>% remove_all_na() #single initial rx, course >1, oral, reviewed >72
 ...
 {         node5a <- node4a %>% 
            filter(Route_1 == "Intravenous") %>% remove_all_na() #double rx, course >=3, both IV
          node5b <- node4a %>% 
            filter(!Route_1 == "Intravenous") %>% remove_all_na() #double rx, course >=3, both oral
 }   
      {  #if not same route, were either stopped before 72 hours (double rx >= 3 in course, different routes)
          node5c <- node4b %>% 
            filter(Prescription_Length_1 <=72 | Prescription_Length_2 <= 72) %>% remove_all_na() # double rx, course >3, different routes, either was stopped <72
          node5d <- node4b %>% 
            filter(!(Prescription_Length_1 <=72 | Prescription_Length_2 <= 72)) %>% remove_all_na() #double rx, course >3, different routes, neither was stopped <72
        #if same route, was it IV (double rx less than 3 in course)
          node5e <- node4c %>% 
            filter(Route_1 == "Intravenous") %>% remove_all_na() #double rx, <3 course, both IV
          node5f <- node4c %>% 
            filter(!Route_1 == "Intravenous") %>% remove_all_na() #double rx, <3 course, both oral
      }     
      {  #if not same route, were either stopped before 72 hours (double rx <3 in course, different routes)
          node5g <- node4d %>% 
          ...
          node5i <- node4e %>% 
            filter(Route_1 == Route_2 & Name_1 == Name_2 & Dose_1 == Dose_2 & Frequency_1 == Frequency_2) #initial IV, course >1, rev < 72, rx kept same
                                                                                                          #this will allow for reviews after 6 hrs to count,
...
{         
          ...
          #careful that interval can be +ve or -ve so need to use between() function
            node6a <- node5c %>% 
              filter(between(interval(Prescription.end_1, Prescription.end_2)/hours(1),-6,6)) %>% remove_all_na() #double rx, course >=3, mixed route, either stopped <72
            ...
            node6c <- node5e %>% 
              filter(Prescription_Length_1 <=72 | Prescription_Length_2 <= 72) %>% remove_all_na() #double rx, course <3, both IV, either stopped before 72hrs 
            node6d <- node5e %>% 
              filter(!(Prescription_Length_1 <=72 | Prescription_Length_2 <= 72)) %>% remove_all_na() #double rx, course <3, both IV, neither stopped before 72hrs            
          ...
          node6k <- node5d %>%
            filter(between(interval(Prescription.end_1, Prescription.end_2)/hours(1),-6,6)) #double rx, course >=3, mixed route, neither stopped <72
                                                                                                        #stopped at same time
            node6l <- node5d %>% 
              filter(!between(interval(Prescription.end_1, Prescription.end_2)/hours(1),-6,6))%>% remove_all_na() #double rx, course >=3, mixed route, neither stopped <72
}                                                                                                        #stopped separately
