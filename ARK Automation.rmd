...
node1b <- abx_per_indication %>% #no
  filter(!is.na(Name_2)) %>% remove_all_na()

#if there was only one prescription was it prescribed for less than 72hours?
  node2a <- node1a %>% 
    filter(Prescription_Length_1 <= 72) %>% remove_all_na() #yes
  
  node2b <- node1a %>% 
    filter(Prescription_Length_1 > 72) %>% remove_all_na()#no
...
nrow(node2c)/ (nrow(node2c)+nrow(node2d))*100  #%of rx courses >=2 that were double rx
      ...
      node3a <- node2d %>% 
        filter(Route_1 == "Intravenous") %>% #yes
        remove_all_na()
      
      node3b <- node2d %>% 
        filter(!Route_1 == "Intravenous") %>% #no
        remove_all_na()
...
nrow(node3a)/ (nrow(node3a)+nrow(node3b))*100 #% of rx courses >=2 that had the first prescription as single IV
      ...
      node3c <- node2c %>% 
        filter(!is.na(Name_3)) %>% remove_all_na() #yes
      
      node3d <- node2c %>% 
        filter(is.na(Name_3)) %>% remove_all_na() #
      #if there was one rx and it was rx for less than 72 hours, was it IV?
      node3e <- node2a %>% 
        filter(Route_1 == "Intravenous") %>% remove_all_na() #yes
      node3f <- node2a %>% 
        filter(!Route_1 == "Intravenous") %>% remove_all_na()    #no  
      ...
      node3h <- node2b %>% 
        ...
        node4a <- node3c %>% 
          filter(Route_1 == Route_2) %>% remove_all_na() #yes
        node4b <- node3c %>% 
          filter(!Route_1 == Route_2) %>% remove_all_na() #no
        ...
        # if same route, was it IV (double rx more than 3 in course)
          node5a <- node4a %>% 
            filter(Route_1 == "Intravenous") %>% remove_all_na() #yes
          node5b <- node4a %>% 
            filter(!Route_1 == "Intravenous") %>% remove_all_na() #no
        #if not same route, were either stopped before 72 hours (double rx more than 3 in course, different routes)
          node5c <- node4b %>% 
            filter(Prescription_Length_1 <=72 | Prescription_Length_2 <= 72) %>% remove_all_na() # yes
          node5d <- node4b %>% 
            filter(!(Prescription_Length_1 <=72 | Prescription_Length_2 <= 72)) %>% remove_all_na() # no
        #if same route, was it IV (double rx less than 3 in course)
          node5e <- node4c %>% 
            filter(Route_1 == "Intravenous") %>% remove_all_na() #yes (double IV, <3 course)
          node5f <- node4c %>% 
            filter(!Route_1 == "Intravenous") %>% remove_all_na() #no )(double oral, <3 course)
          
           #if same route, were either stopped before 72 hours (double rx < 3 in course, different routes)
          node5g <- node4c %>% 
            filter(Prescription_Length_1 <=72 | Prescription_Length_2 <= 72) %>% remove_all_na() # yes
          node5h <- node4c %>% 
            filter(!(Prescription_Length_1 <=72 | Prescription_Length_2 <= 72)) %>% remove_all_na() # no
          ...
          # if not same route and either stopped before 72 hours, were they both stopped at the same time? i.e within 6 hours of each other  (double rx more than 3 in course, different routes)
            node6a <- node5c %>% 
              filter(interval(Prescription.end_1, Prescription.end_2)/hours(1)<= 1) %>% remove_all_na() # yes
            node6b <- node5c %>% 
              filter(!interval(Prescription.end_1, Prescription.end_2)/hours(1)<= 1)%>% remove_all_na() # no
              ...
              node7a <- node6b %>% 
                ...
                remove_all_na() # yes
              node7b <- node6b %>% 
                filter(!(Route_1 == "Intravenous" & Prescription.end_1 < Prescription.end_2 | Route_2 == "Intravenous" & Prescription.end_2<Prescription.end_1)) %>% remove_all_na() #no
